...
Puppet::Type.type(:aem_certificate).provide(:aem, parent: PuppetX::ShineSolutions::PuppetAemResources) do
  ...
  def create
    return false if resource[:keystore_password].nil?
    return false if resource[:truststore_password].nil?
    return false if resource[:privatekey_file].nil?
    return false if resource[:certificate_file].nil?
    ...
    result = ssl.enable(resource[:keystore_password], resource[:truststore_password], resource[:https_hostname], resource[:https_port], resource[:privatekey_file], resource[:certificate_file])
